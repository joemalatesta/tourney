...
function adjustPlayerStats(data) {
  ...
  let winner = data.winningPlayer._id === data.player1._id ? data.player1 : data.player2
  let loser = data.losingPlayer._id === data.player1._id ? data.player1 : data.player2 
...
function winningPlayer(winner, loser) {
  ...
  let updatedPlayer = {
    ...
    gamesWon: winner.gamesWon + winner.gamesWon.reduce((acc, value) => {
      if(value === true) acc += 1
      return acc
    },0),
    gamesLoss: winner.gamesLoss + loser.gamesWon.reduce((acc, value) => {
      if(value === true) acc += 1
      return acc
    },0),
  }
...
function losingPlayer(loser, winner) {
  ...
  let updatedPlayer = {
    ...
    gamesWon: loser.gamesWon + loser.gamesWon.reduce((acc, value) => {
      if(value === true) acc += 1
      return acc
    },0),
    gamesLoss: loser.gamesLoss + winner.gamesWon.reduce((acc, value) => {
      if(value === true) acc += 1
      return acc
    },0),
  }
